...
class NeuralNet_3dof():
    ...
    def auto_diff(self,cur_state_tf, desired_next_pose, deriv_cur_state_tf, desired_next_vel, pred_next_state):
        ...
        n_th1_tt = tf.expand_dims(tf.reduce_sum(tf.gradients(pred_next_state[:,3],cur_state_tf)[0]*deriv_cur_state_tf,1)+tf.reduce_sum(tf.gradients(pred_next_state[:,3],desired_next_pose)[0]*desired_next_vel,1),1)
        n_th2_tt = tf.expand_dims(tf.reduce_sum(tf.gradients(pred_next_state[:,4],cur_state_tf)[0]*deriv_cur_state_tf,1)+tf.reduce_sum(tf.gradients(pred_next_state[:,4],desired_next_pose)[0]*desired_next_vel,1),1)
        n_th3_tt = tf.expand_dims(tf.reduce_sum(tf.gradients(pred_next_state[:,5],cur_state_tf)[0]*deriv_cur_state_tf,1)+tf.reduce_sum(tf.gradients(pred_next_state[:,5],desired_next_pose)[0]*desired_next_vel,1),1)
        
        return tf.concat([n_th1_t, n_th2_t, n_th3_t, n_th1_tt, n_th2_tt, n_th3_tt],1)
    ...
    def build_graph(self):
        ...
        self.pose = tf.placeholder(tf.float32, [None, 3], name="pose")
        self.vel = tf.placeholder(tf.float32, [None, 3], name="vel")

        self.next_pose = tf.placeholder(tf.float32, [None, 3], name="next_pose")
        self.next_vel = tf.placeholder(tf.float32, [None, 3], name="next_vel")

        self.desired_next_pose = tf.placeholder(tf.float32, [None, 3], name="desired_next_pose")
        ...
        if self.deriv:
            self.desired_next_vel = tf.placeholder(tf.float32, [None, 3], name="desired_next_vel")
            self.desired_acc = tf.placeholder(tf.float32, [None, 3], name="desired_acc")
            self.desired_next_acc = tf.placeholder(tf.float32, [None, 3], name="desired_next_acc")
    ...
    def train(self, epoch, train_data, eval_data, exp_name, save=True, eval_interval=100):
        ...
        for i in range(epoch):
            if self.deriv:
                feed_dict = {self.pose:np.asarray(train_data['real_cur_pos'])[:,0:3],
                             self.vel:np.asarray(train_data['real_cur_vel'])[:,0:3],
                             self.desired_acc:np.asarray(train_data['desired_cur_acc'])[:,0:3],
                             self.next_pose:np.asarray(train_data['real_next_pos'])[:,0:3],
                             self.next_vel:np.asarray(train_data['real_next_vel'])[:,0:3],
                             self.desired_next_acc:np.asarray(train_data['desired_next_acc'])[:,0:3],
                             self.desired_next_pose:np.asarray(train_data['desired_next_pos'])[:,0:3],
                             self.desired_next_vel:np.asarray(train_data['desired_next_vel'])[:,0:3]}
    ...
    def evaluation(self, eval_data):
        ...
        if self.deriv:
            feed_dict ={self.pose:np.asarray(eval_data['real_cur_pos'])[:,0:3],
                        self.vel:np.asarray(eval_data['real_cur_vel'])[:,0:3],
                        self.desired_acc:np.asarray(eval_data['desired_cur_acc'])[:,0:3],
                        self.next_pose:np.asarray(eval_data['real_next_pos'])[:,0:3],
                        self.next_vel:np.asarray(eval_data['real_next_vel'])[:,0:3],
                        self.desired_next_acc:np.asarray(eval_data['desired_next_acc'])[:,0:3],
                        self.desired_next_pose:np.asarray(eval_data['desired_next_pos'])[:,0:3],
                        self.desired_next_vel:np.asarray(eval_data['desired_next_vel'])[:,0:3]}
    ...
    def predict(self, pose, vel, desired_next_pose):
        # data를 이용하여 feed
        feed_dict ={self.pose:pose[:,0:3],
                    self.vel:vel[:,0:3],
                    self.desired_next_pose:desired_next_pose[:,0:3]}
...
class NeuralNet_Manipulability():
    ...
    def train(self, epoch, train_data, eval_data, save=True, eval_interval=100):
        ...
        for i in range(epoch):
            feed_dict = {self.pose:train_data['real_cur_pos'],
                        self.m_index:np.expand_dims(train_data['real_m_index'],1)}
    ...
    def evaluation(self, eval_data):
        feed_dict = {self.pose:eval_data['real_cur_pos'],
                    self.m_index:np.expand_dims(eval_data['real_m_index'],1)}
